...
import { ArrowLeft, FileImage, LoaderCircle, Send, ShieldCheck, Upload } from "lucide-react";
...
const configurations = {
  ...
  "/report-analyzer": {
    ...
    description: "Upload a clear image of a medical report to receive a plain-language educational summary.",
    instruction: "Summarize the visible report in plain language, explain medical terms, identify values that may need discussion with a clinician, and state when the image is unclear. Do not diagnose.",
  },
};
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        ...
        <form onSubmit={analyze} className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <label className="mt-3 flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center hover:border-cyan-500">
            ...
            <span className="mt-3 text-sm font-semibold">{file ? file.name : "Choose a JPG, PNG, WEBP, HEIC, or HEIF image"}</span>
            <span className="mt-1 text-xs text-slate-500">Maximum file size: 10 MB</span>
            <input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => setFile(event.target.files?.[0] || null)} className="sr-only" />
          </label>
          ...
          <button disabled={loading} className="mt-6 inline-flex items-center gap-2 rounded-full bg-cyan-700 px-5 py-3 text-sm font-semibold text-white hover:bg-cyan-800 disabled:cursor-not-allowed disabled:opacity-60">{loading ? <LoaderCircle className="animate-spin" size={17} /> : <Send size={17} />} {loading ? "Analyzing..." : "Analyze image"}</button>
        </form>
